...
class PrivAgentBackground {
  constructor() {
    ...
    this.privacyEngine = {
        ...
        for (const [type, pattern] of Object.entries(patterns)) {
          ...
          if (matches) {
            matches.forEach(match => {
              sensitiveData.push({ type, value: match, position: text.indexOf(match) });
              filteredText = filteredText.replace(match, `[${type.toUpperCase()}_REDACTED]`);
            });
          }
        }
      ...
      getStats: () => ({
        totalItemsProcessed: this.privacyStats.totalDataFiltered || 0,
        sensitiveItemsFiltered: this.privacyStats.totalDataFiltered || 0,
  ...
  initializeExtension() {
    ...
    // Monitor navigation for privacy context (if webNavigation is available)
    if (chrome.webNavigation && chrome.webNavigation.onCompleted) {
      chrome.webNavigation.onCompleted.addListener((details) => {
        if (details.frameId === 0) { // Main frame only
          this.analyzePageContext(details.tabId, details.url);
        }
      });
    }
  }
  ...
  getComprehensiveStats() {
    const engineStats = this.privacyEngine.getStats();
    return {
      ...engineStats,
      ...this.privacyStats,
      timestamp: Date.now(),
      privacyScore: this.calculatePrivacyScore(engineStats)
    };
  }
